...
function ShareRow({ label, path }: { label: string; path: string }) {
  ...
  return (
    <div className="flex w-full gap-2 items-center">
      ...
      <button
        onClick={doCopy}
        className="px-3 py-1 rounded bg-blue-600 text-white text-sm hover:bg-blue-700 active:scale-[0.99]"
      >
        {copied ? "Copied!" : "Copy"}
      </button>
    </div>
...
export default function BuilderPage() {
  ...
  const [status, setStatus] = useState<FormDoc["status"]>("draft");
  ...
  const addField = (t: FieldType) => {
    setFields((f) => [...f, (t === "text") ? emptyText()
  ...
  const doSave = useCallback(async () => {
    ...
    try {
      ...
      const res = formId ? await updateForm(formId, doc) : await createForm(doc);
      setFormId(res.id);
          ...
          <div key={f.key}
            ...
            <div className="grid sm:grid-cols-3 gap-3 mt-3">
              <div>
                ...
                <select className="w-full border rounded px-2 py-2" value={f.type}
                        onChange={e=>updateField(f.key, { type: e.target.value as any })}>
              ...
              {f.type === "rating" && (
                <div>
                  ...
                  <input type="number" min={1} className="w-full border rounded px-3 py-2"
                         value={f.max ?? 5} onChange={e=>updateField(f.key, { max: Number(e.target.value) })}/>
                </div>
              )}
            </div>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap gap-2 items-center">
          <button className="px-4 py-2 rounded bg-black text-white disabled:opacity-50"
                  disabled={!canSave || saving} onClick={doSave}>
